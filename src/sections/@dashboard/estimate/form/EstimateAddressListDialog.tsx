...
// @types
import { EEstimateAddress } from '../../../../@types/estimate';
// components
...
type Props = {
  ...
  onSelect: (address: EEstimateAddress | null) => void;
  addressOptions: EEstimateAddress[];
};

export default function EstimateAddressListDialog({
  open,
  selected,
  onClose,
  onSelect,
  ...
  const handleSelectAddress = (address: EEstimateAddress | null) => {
    onSelect(address);
    setSearchAddress('');
    onClose();
  };
  ...
  return (
    <Dialog fullWidth maxWidth="xs" open={open} onClose={onClose}>
      <Stack
        ...
        sx={{ pt: 2.5, px: 3 }}
      >
        <Typography variant="h6"> Sélectionner une adresse </Typography>
...
function applyFilter(array: EEstimateAddress[], query: string) {
  if (query) {
    return array.filter(
      (address) =>
