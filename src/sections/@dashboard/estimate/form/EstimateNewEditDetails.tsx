...
// @types
import { EEstimateItem } from '../../../../@types/estimate';
// components
...
export default function EstimateNewEditDetails() {
  ...
  const totalOnRow = values.items.map((item: EEstimateItem) => item.quantity * item.price);
  ...
  const handleSelectService = useCallback(
    (index: number, option: string) => {
      ...
      setValue(
        `items[${index}].total`,
        values.items.map((item: EEstimateItem) => item.quantity * item.price)[index]
      );
    },
